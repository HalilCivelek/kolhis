...
=>
    ...
    const bodyTemplate = (data, props) => {
        return (
            < >
            < span
        className = "p-column-title" > {props.header} < /span>
        {
            data[props.field]
        }
    <
    ...
    const countryBodyTemplate = (data) => {
        return (
            < >
            < span
        className = "p-column-title" > Country < /span>
            < img
        src = "assets/demo/images/flags/flag_placeholder.png"
        alt = {data.country.name}
        className = {`flag flag-${data.country.code}`
    }
        width = {30}
        height = {20}
        />
        ...
        style = {
        {
            marginLeft: '.5em', verticalAlign
        :
            'middle'
        }
    }
        className = "image-text" > {data.country.name} < /span>
            < />
    )
        ;
    };
    ...
    const representativeBodyTemplate = (data) => {
        return (
            < >
            < span
        className = "p-column-title" > Representative < /span>
            < img
        ...
        className = "image-text" > {data.representative.name} < /span>
            < />
    )
        ;
    };
    ...
    const statusBodyTemplate = (data) => {
        return (
            < >
            < span
        className = "p-column-title" > Status < /span>
            < span
        className = {`customer-badge status-${data.status}`
    }>
        {
            data.status
        }
    ...
    const activityBody = (data) => {
        return (
            < >
            < span
        className = "p-column-title" > Activity < /span>
            < ProgressBar
        value = {data.activity}
        showValue = {false}
        />
...
<
    ...
    const imageBodyTemplate = (data) => {
        return (
            < >
            < span
        className = "p-column-title" > Image < /span>
            < img
    ...
    const priceBodyTemplate = (data) => {
        return (
            < >
            < span
        className = "p-column-title" > Price < /span>
        {
    ...
    const reviewsBodyTemplate = (data) => {
        return (
            < >
            < span
        className = "p-column-title" > Reviews < /span>
            < Rating
        value = {data.rating}
        readonly
        cancel = {false}
    ...
    const footerRowGroup = (data) => {
        ...
        /strong></
        td >
        < td > < strong > {calculateCustomerTotal(data.representative.name
    )
    }<
        /strong></
        td >
...
=>
    ...
    header = "Country"
    sortable
    body = {countryBodyTemplate} > < /Column>
        < Column
    ...
    header = "Activity"
    sortable
    body = {activityBody} > < /Column>
        < Column
    headerStyle = {
    {
        width: '8rem', textAlign
    :
        'center'
    }
}
    bodyStyle = {
    {
        textAlign: 'center', overflow
    :
        'visible', justifyContent
    :
        'center'
    }
}
    ...
    table
    with gridlines( < mark >.
    p - datatable - gridlines < /mark>), striped rows (<mark>.p-datatable-striped</m
    ark >
)
    and
    smaller
    paddings( < mark > p - datatable - sm < /mark>).</
    p >
    < DataTable
    value = {customer2}
    paginator
...
=>
    ...
    header = "Reviews"
    sortable
    body = {reviewsBodyTemplate} > < /Column>
        < Column
